...
class HodgkinHuxleyModel:
    ...
    def model_pk2_zukov_original(self, t: float, y: np.ndarray) -> np.ndarray:
        V, n, m, h = y
        ...
        dVdt_corr = dVdt + self.dt * (dVdt_corr - dVdt)
        dndt_corr = dndt + self.dt * (dndt_corr - dndt)
        dmdt_corr = dmdt + self.dt * (dmdt_corr - dmdt)
        dhdt_corr = dhdt + self.dt * (dhdt_corr - dhdt)
    ...
    def simulate(self, method: str = 'RK45') -> Tuple[np.ndarray, np.ndarray]:
        ...
        elif method == 'PK2_Zukov_Original':
            sol = solve_ivp(self.model_pk2_zukov_original, [self.t_array[0], self.t_array[-1]], y0, t_eval=self.t_array)
        else:
...
def compare_models(params: Dict[str, float], t_max: float, dt: float, I_ext: np.ndarray):
    ...
    t_pk2_zukov_original, V_pk2_zukov_original = hh_model.simulate(method='PK2_Zukov_Original')
    ...
    V_pk2_zukov_original_interp = np.interp(t_rk45, t_pk2_zukov_original, V_pk2_zukov_original)
    ...
    # Calculate metrics for PK2_Zukov_Original method
    mse_pk2_zukov_original = np.mean((V_pk2_zukov_original_interp - V_rk45) ** 2)
    mae_pk2_zukov_original = np.mean(np.abs(V_pk2_zukov_original_interp - V_rk45))
    rmse_pk2_zukov_original = np.sqrt(mse_pk2_zukov_original)
    r2_pk2_zukov_original = 1 - (mse_pk2_zukov_original / np.var(V_rk45))
    mape_pk2_zukov_original = np.mean(np.abs((V_pk2_zukov_original_interp - V_rk45) / V_rk45)) * 100
    ...
    print(f"Method PK2 Zukov Original:MSE = {mse_pk2_zukov_original:.4f}, MAE = {mae_pk2_zukov_original:.4f}, RMSE = {rmse_pk2_zukov_original:.4f}, R2 = {r2_pk2_zukov_original:.4f}, MAPE = {mape_pk2_zukov_original:.4f}%")
    ...
    plt.plot(t_pk2_zukov_original, V_pk2_zukov_original_interp, label='PK2 Zukov Original', color='orange', linestyle='-')
    plt.xlabel('Time (ms)')
